...
static inline unsigned int get_boot_number(void)
{
    std::string path;

    path = (std::getenv("PERSISTENT_LOG_DIR") ?: std::string("/obfl")) + \
            std::string("/") + std::string("boot_count.txt");
...
inline void spdlog::logger::log(level::level_enum lvl, const char* fmt, const Args&... args)
{
    ...
    try
    {
        details::log_msg log_msg(_boot_number, &_name, lvl);
...
inline void spdlog::logger::_default_err_handler(const std::string &msg)
{
    ...
    err_msg.formatted.write("[*** LOG ERROR ***] [{}] [{}] [{}{}]{}", name(), msg, date_buf,tm_now.tv_usec, details::os::eol);
